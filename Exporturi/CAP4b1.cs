...
namespace exportXml.Exporturi
{
    public class CAP4b1
    {
        public static bool make_CAP4b1xml(string strIdRol)
        {
            try
            {
                string strGosp = strIdRol.Substring(0, strIdRol.Length - 3);

                if (File.Exists(AppDomain.CurrentDomain.BaseDirectory.ToString() + "XML\\Cap4b1\\" + AjutExport.numefisier(strIdRol) + "xml") == true)
                {
                ...
                //baza de date--
                strSQL = "SELECT ROL.nrcrt, Cap4b.sup FROM Cap4b LEFT JOIN (SELECT * FROM NOMCap4b) AS ROL ON Cap4b.NrCrt = ROL.NrCrt WHERE Cap4b.IDROL=\"" + strIdRol + "\"  ORDER BY ROL.nrcrt;";
                ...
                //DOCUMENT_RAN
                XmlWriter xmlWriter = XmlWriter.Create(AppDomain.CurrentDomain.BaseDirectory.ToString() + "XML\\Cap4b1\\" + strGosp + "xml", settings);
                ...
                xmlWriter.WriteStartElement("capitol_4b1");                  //capitol
                xmlWriter.WriteAttributeString("codCapitol", "Cap4b1");
                xmlWriter.WriteAttributeString("denumire", "Suprafața cultivată în sere pe raza localității");
                //--

                //parcurg--
                while (drXML.Read())
                {
                    xmlWriter.WriteStartElement("cultura_in_sere");    //rand 
