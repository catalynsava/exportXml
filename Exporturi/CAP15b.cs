...
namespace exportXml.Exporturi
{
    public class CAP15b
    {
         public static bool make_CAP15b(string strIdRol)
        {
            try
            {
                int nrcrt = 1;
                ...
                if (File.Exists(AppDomain.CurrentDomain.BaseDirectory.ToString() + "XML\\CAP15b\\" + AjutExport.numefisier(strIdRol) + "xml") == true)
                {
                ...
                //baza de date--
                strSQL = "SELECT * FROM CAP15b WHERE IDROL=\"" + strIdRol + "\" AND tippers=\"CONCESIONAR\" ORDER BY nrContr, data;";
                ...
                //-- 
                XmlWriter xmlWriter = XmlWriter.Create(AppDomain.CurrentDomain.BaseDirectory.ToString() + "XML\\CAP15b\\" + strGosp + "xml", settings);
                ...
                xmlWriter.WriteStartElement("capitol_15b");         //deschid capitol_15a
                xmlWriter.WriteAttributeString("codCapitol", "CAP15b");
                xmlWriter.WriteAttributeString("denumire", "Inregistrari privind contractele de concesiune");
                 ...
                 while (drXML.Read())
                {
                    ...
                    xmlWriter.WriteStartElement("contract_concesiune");   //deschid contract_arendare

                    xmlWriter.WriteElementString("codCategFolosinta", "DCF");
                    ...
                    nrcrt += 1;
                    xmlWriter.WriteElementString("redeventaLei", "0");
                    xmlWriter.WriteStartElement("suprafataMP");

                    xmlWriter.WriteAttributeString("value", (AjutExport.getSuprafataContrConcesiune(drXML["nrContr"].ToString(),drXML["data"].ToString())).ToString());
                    ...
                    if (ePJ==false)
                    {
                        xmlWriter.WriteStartElement("concesionarPF");
                    ...
                    {
                        xmlWriter.WriteStartElement("concesionarRC");       //START concesionarRC
                        ...
                        xmlWriter.WriteEndElement();        //inchid concesionarRC
